...
const Video = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!hasViewed) {
          await axios.put(`http://localhost:4005/api/videos/view/${currentVideo._id}`);
          setHasViewed(true);
        }


        const videoRes = await axios.get(`http://localhost:4005/api/videos/find/${path}`)
        const chanelRes = await axios.get(`http://localhost:4005/api/users/find/${videoRes.data.userId}`);
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      await axiosForLike.put(`http://localhost:4005/api/users/like/${currentVideo._id}`)
  ...
  const handleDislike = async () => {
    ...
    try {
      ...
      await axiosForDislike.put(`http://localhost:4005/api/users/dislike/${currentVideo._id}`)
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      currentUser.subscribedUsers?.includes(chanel._id)
        ...
        ? await axiosForSub.put(`http://localhost:4005/api/users/unsub/${chanel._id}`)



        : await axiosForSub.put(`http://localhost:4005/api/users/sub/${chanel._id}`);
